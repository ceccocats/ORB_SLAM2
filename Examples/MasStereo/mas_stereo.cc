...
int main(int argc, char **argv)
{
    ...
    std::ifstream timestamps(argv[5]);

    VideoCapture capL(argv[3]);  
    VideoCapture capR(argv[4]);  

    // Main loop
    cv::Mat origL, origR, imLeft, imRight, imLeftRect, imRightRect;
    while(capL.isOpened() && capR.isOpened()) {

        capL >> origL;
        capR >> origR;
        if(origL.empty() || origR.empty()) {
            std::cout<<"read fail!!\n";        
            continue;
        }

        resize(origL, imLeft, Size(), 0.5, 0.5);
        resize(origR, imRight, Size(), 0.5, 0.5);
